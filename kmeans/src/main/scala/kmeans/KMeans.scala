...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointsGroupByMean = points.groupBy(findClosest(_, means))
    means.map(m => (m, pointsGroupByMean.getOrElse(m, GenSeq.empty[Point]))).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans map {
      om =>
        findAverage(om, classified.getOrElse(om, GenSeq.empty[Point]))
    }
  }
