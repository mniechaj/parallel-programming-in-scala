...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val half = (end - from) / 2
      val (l, r) = parallel(upsweep(input, from, half, threshold), upsweep(input, half, from, threshold))
      Node(l ,r)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    (from until until) foreach {
      idx =>
        if (idx == 0) output(0) = 0
        else if (idx == from) output(idx) = max(input(idx) / idx, startingAngle)
        else output(idx) = max(input(idx) / idx, output(idx - 1))
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = tree match {
    ...
    case Node(l, r) =>
      parallel(
        ...
        downsweep(input, output, max(l.maxPrevious, startingAngle), r))
  }
