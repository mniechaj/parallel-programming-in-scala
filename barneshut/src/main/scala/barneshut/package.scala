...
package object barneshut {
  ...
  extends Quad {
    ...
    val massX: Float = bodies.foldLeft(0f)((sum, body) => sum + body.mass * body.x) / mass
    val massY: Float = bodies.foldLeft(0f)((sum, body) => sum + body.mass * body.y) / mass
    ...
    def insert(b: Body): Quad = if (size <= minimumSize) {
      ...
      val nwCenterX = centerX - centerOffset
      val nwCenterY = centerY - centerOffset
      val neCenterX = centerX + centerOffset
      val neCenterY = nwCenterY
      val swCenterX = nwCenterX
      val swCenterY = centerY + centerOffset
      val seCenterX = neCenterX
      val seCenterY = swCenterY

      val fork = Fork(
        Empty(nwCenterX, nwCenterY, newSize),
        Empty(neCenterX, neCenterY, newSize),
        Empty(swCenterX, swCenterY, newSize),
        Empty(seCenterX, seCenterY, newSize)
      )
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for (idx <- matrix.indices) {
        matrix.update(idx, this.matrix(idx).combine(that.matrix(idx)))
      }
      this
    }
